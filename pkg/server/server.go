...
import (
	...
	"github.com/jlison/slack-mcp-server/pkg/handler"
	"github.com/jlison/slack-mcp-server/pkg/provider"
	"github.com/jlison/slack-mcp-server/pkg/server/auth"
	"github.com/jlison/slack-mcp-server/pkg/text"
	"github.com/jlison/slack-mcp-server/pkg/version"
...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger) *MCPServer {
	...
	s.AddTool(mcp.NewTool("channels_list",
		...
		mcp.WithNumber("limit",
			...
			mcp.Description("The maximum number of items to return. Must be an integer between 1 and 1000 (maximum 999)."), // context fix for cursor: https://github.com/jlison/slack-mcp-server/issues/7
		),
		mcp.WithString("cursor",
			mcp.Description("Cursor for pagination. Use the value of the last row and column in the response as next_cursor field returned from the previous request."),
		),
